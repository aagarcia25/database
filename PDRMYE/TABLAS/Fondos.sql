...
-- Volcando estructura para tabla PDRMYE.Fondos
...
CREATE TABLE IF NOT EXISTS `Fondos` (
  ...
  `AplicaCalculo` int(1) NOT NULL,
  `Vigente` int(1) NOT NULL,
  `Estatal` int(1) NOT NULL DEFAULT 0,
  `Federal` int(1) NOT NULL DEFAULT 0,
  `Tipo` char(36) DEFAULT NULL,
  `PorcentajeDistribucion` decimal(20,2) NOT NULL DEFAULT 0.00,
  `Garantia` binary(1) DEFAULT '0',
  `Articulo` int(1) DEFAULT NULL,
  ...
  `Aportacion` tinyint(1) DEFAULT NULL,
  `Egreso` int(1) DEFAULT 1,
  PRIMARY KEY (`id`) USING BTREE,
  KEY `FK1_TipoFondo` (`Tipo`),
  CONSTRAINT `FK1_TipoFondo` FOREIGN KEY (`Tipo`) REFERENCES `TipoFondos` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_general_ci COMMENT='tabla para guardar información de los Fondos';
